...
PetscErrorCode c3vinterp(MPI_Comm comm, Mat *Aout, int Mx, int My, int Nx, int Ny)
{
  ...
  for (i = ns; i < ne; ++i) {
    ...
    if(ix1>=0 && ix2<Nx && iy1>=0 && iy2<Ny){
      ierr = bilinearinterp(x3-0.5,y3-0.5,Ny,&p11,&p21,&p12,&p22,&w11,&w21,&w12,&w22);
  ...
  MatDuplicate(A,MAT_COPY_VALUES,&Atmp);
  int rstart, rend, row;
  PetscInt ncols;
  const PetscInt *cols;
  ...
  for (row=rstart; row<rend; row++) {
    ...
    for (j=0; j<ncols; j++){
      if(vals[j]>1.0){
        MatSetValue(Atmp,row,cols[j],1,INSERT_VALUES);
      }
    }
    MatRestoreRow(A,row,&ncols,&cols,&vals);
  }
  MatAssemblyBegin(Atmp,MAT_FINAL_ASSEMBLY);
  MatAssemblyEnd(Atmp,MAT_FINAL_ASSEMBLY);
  MatCopy(Atmp,A,SAME_NONZERO_PATTERN);
  MatDestroy(&Atmp);
  /**********************/
