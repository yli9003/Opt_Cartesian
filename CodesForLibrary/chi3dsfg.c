...
double chi3dsfg(int DegFree,double *epsopt, double *grad, void *data)
{
  ...
  PetscPrintf(PETSC_COMM_WORLD,"----Third Order Sum Frequency Generation. Minapporoach NOT Available. ------- \n");
  
  Vec x3,J3,wtJ3,wtJ3c,b3;
  Vec Bx1,Bx2,Bx1c,Bx2c,tmp,tmp1,tmp2,Bx2sq,Bx2sqc,Bx12c;
  ...
  VecDuplicate(vR,&Bx2sq);
  VecDuplicate(vR,&Bx2sqc);
  VecDuplicate(vR,&Bx12c);
  VecDuplicate(vR,&u1);
  VecDuplicate(vR,&u2);
  VecDuplicate(vR,&u3);
  VecDuplicate(vR,&u4);
  ...
  MatMult(B1,x1,Bx1);
  MatMult(B2,x2,Bx2);
  CmpVecProd(Bx2,Bx2,Bx2sq);
  CmpVecProd(Bx1,Bx2sq,J3);
...
/*------------------------------------------------*/
 ...
 /*-------take care of the gradient---------*/
  if (grad) {
    MatMult(C,Bx2sq,Bx2sqc);
    CmpVecProd(Bx2sqc,x3,tmp1);
    ...
    MatMult(C,Bx1,Bx1c);
    MatMult(C,Bx2,Bx2c);
    CmpVecProd(Bx1c,Bx2c,Bx12c);
    VecScale(Bx12c,2.0);
    CmpVecProd(Bx12c,x3,tmp2);
