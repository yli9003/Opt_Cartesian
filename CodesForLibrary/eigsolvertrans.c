...
int eigsolvertrans(Mat M, Vec epsC, Mat D, Mat C)
{
  ...
  ierr=MatCreateVecs(M,PETSC_NULL,&tmpr); CHKERRQ(ierr);
  ierr=MatCreateVecs(M,PETSC_NULL,&tmpi); CHKERRQ(ierr);
  int ni;
  for(ni=0; ni<nconv; ni++)
    {
      ierr=EPSGetEigenpair(eps, ni, krarray+ni, kiarray+ni, tmpr, tmpi);CHKERRQ(ierr);
