		...
		{for (k=0; k<Nz; k++)
			...
			{pos = i*Ny*Nz + j*Nz + k;
			  if ( ns < pos+dir*N+1 && ne > pos+dir*N){
			    ...
			    if (chiefray==0){
			      phi = refphi - (2*PI/lambda) * (sqrt(pow(fcl*tan(theta_inc)-dl,2)+pow(fcl,2))-fcl);
			    }else if(chiefray==1){
			      phi = refphi - (2*PI/lambda) * (sqrt(pow(fcl*tan(theta_inc)-dl,2)+pow(fcl,2))-fcl/cos(theta_inc));
			    }else{
			      phi = refphi - (2*PI/lambda) * (sqrt(pow(dl,2)+pow(fcl,2))-fcl);
			    }
			    ampr=cos(phi);
			    ampi=sin(phi);
...
//Note lambda means how many pixels are in one wavelength
#undef __FUNCT__ 
#define __FUNCT__ "makepq_linear"
		...
		{for (k=0; k<Nz; k++)
			...
			{pos = i*Ny*Nz + j*Nz + k;
			  if ( ns < pos+dir*N+1 && ne > pos+dir*N){
			    PetscPrintf(comm,"DEBUG: I AM HERE IN makepq_lens.\n");
 			    dl=((ux-lx>1)*(i-ix0) + (uy-ly>1)*(j-iy0) + (uz-lz>1)*(k-iz0));
			    phi = refphi - (2*PI/lambda) * slope * dl;
			    ampr=cos(phi);
			    ampi=sin(phi);
			    PetscPrintf(comm,"DEBUG: ampr %g, ampi %g \n",ampr,ampi);
			    if(dir==0) {
