...
#undef __FUNCT__ 
#define __FUNCT__ "sfg_graphene"
double sfg_graphene(int DegFree,double *epsopt, double *grad, void *data)
{
  
  PetscErrorCode ierr;

  SFGdataGroupGraphene *ptdata = (SFGdataGroupGraphene *) data;
...
/*------------------------------------------------*/
 ...
 /*-------take care of the gradient---------*/
  if (grad) {
    ...
    VecPointwiseMult(tmp1,tmp1,vecNL);
    ...
    VecPointwiseMult(tmp2,tmp2,vecNL);
