...
#undef __FUNCT__ 
#define __FUNCT__ "projsimpoverlap"
double projsimpoverlap(int DegFree,double *epsopt, double *grad, void *data)
{
  
  PetscErrorCode ierr;

  PetscPrintf(PETSC_COMM_WORLD,"********Entering the Proj & SIMP OverlapSolver (Full Vectorial Version). Minimum approach available.********** \n");
  ...
  beta=tmpbeta*hxyz;
    ...
    if(minapproach)
    {
      PetscPrintf(PETSC_COMM_WORLD,"---The current overlap (minapp) at step %.5d is %.16e \n", count,beta);
      beta = 1.0/beta;
      PetscPrintf(PETSC_COMM_WORLD,"---The current invoverlap (minapp) at step %.5d is %.16e \n", count,beta);
    }
  else
...
/*------------------------------------------------*/
 ...
 /*-------take care of the gradient---------*/
  if (grad) {
    ...
    if(minapproach){
      VecScale(epsgrad,-1.0*beta*beta);
    }
