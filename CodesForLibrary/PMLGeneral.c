...
#undef __FUNCT__ 
#define __FUNCT__ "EpsPMLGeneral"
...
{  
  ...
  int Nxyz[3]={Nx,Ny,Nz};
  int LowerPML[3]={LowerPMLx,LowerPMLy,LowerPMLz};
  ...
  for (i = ns; i < ne; ++i) {
    ...
    if (npmlic==0)
      ...
      dic =  (LowerPML[ic])* (ixyz[ic] < Npml[ic]) * (Npml[ic]-ixyz[ic]-0.5)/npmlic 
  ...
  /* first -1 is for the postion of E, second -1 is for C index from 0 */
    ...
    if (npmlcp1==0)
      ...
      dcp1 =  (LowerPML[cp1])* (ixyz[cp1] < Npml[cp1] ) * (Npml[cp1]-ixyz[cp1])/npmlcp1	+ ( ixyz[cp1] > (Nxyz[cp1]-Npml[cp1]-1))* (ixyz[cp1] - (Nxyz[cp1]-Npml[cp1]-1))/npmlcp1;
    
    if (npmlcp2==0)
      dcp2=0;
    else
      dcp2 =  (LowerPML[cp2])* (ixyz[cp2] < Npml[cp2] ) * (Npml[cp2]-ixyz[cp2])/npmlcp2	+ (ixyz[cp2] > (Nxyz[cp2]-Npml[cp2]-1))* (ixyz[cp2] - (Nxyz[cp2]-Npml[cp2]-1))/npmlcp2;
...
#undef __FUNCT__ 
#define __FUNCT__ "MuinvPMLGeneral"
PetscErrorCode MuinvPMLGeneral(MPI_Comm comm, Vec *muinvout, int Nx, int Ny, int Nz, int Npmlx, int Npmly, int Npmlz, double sigmax, double sigmay, double sigmaz, double omega, int LowerPMLx, int LowerPMLy, int LowerPMLz)
{
  ...
  for (i = ns; i < ne; ++i) {
    ...
    if (npmlic==0)
      ...
      dic = (LowerPML[ic])* (ixyz[ic] < Npml[ic] ) * (Npml[ic]-ixyz[ic])/npmlic
    ...
    if (npmlcp1==0)
      ...
      dcp1 =  (LowerPML[cp1])* (ixyz[cp1] < Npml[cp1] ) * (Npml[cp1]-ixyz[cp1]-0.5)/npmlcp1	+ ( ixyz[cp1] > (Nxyz[cp1]-Npml[cp1]-1-1))* (ixyz[cp1] - (Nxyz[cp1]-Npml[cp1]-1-1) -0.5)/npmlcp1;
    /* first -1 is for the postion of H, second -1 is for C index from 0 */

    if (npmlcp2==0)
      dcp2 = 0;
    else
      dcp2 =  (LowerPML[cp2])* (ixyz[cp2] < Npml[cp2] ) * (Npml[cp2]-ixyz[cp2]-0.5)/npmlcp2 	+ (ixyz[cp2] > (Nxyz[cp2]-Npml[cp2]-1-1))* (ixyz[cp2] - (Nxyz[cp2]-Npml[cp2]-1-1) -0.5)/npmlcp2;
