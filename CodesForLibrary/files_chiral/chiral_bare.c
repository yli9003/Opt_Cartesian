...
double ldoskdiff(int DegFreeAll,double *epsoptAll, double *gradAll, void *data)
{
  ...
  PetscPrintf(PETSC_COMM_WORLD,"---*****The current non-squared ldos difference between left and right at omega %.4e at step %.5d is %.16e \n", omega/(2*PI),count,ldosdiff);
  ...
  if (chiralweight)
    return epsoptAll[DegFreeAll-1] - chiralweight*ldosdiff;
  else
    return ldosdiff;
}
