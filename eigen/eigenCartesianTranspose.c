...
int main(int argc, char **argv)
{
  ...
  PetscPrintf(PETSC_COMM_WORLD,"BEWARE: THIS IS SOLVING THE RIGHT EIGENVECTORS, I.E. H^T X = l X \n");
  PetscErrorCode ierr;
...
/*-------------------------------------------------------------------------*/
  ...
  //MatTranspose(M,MAT_REUSE_MATRIX,&M);
  //eigsolvertrans(M,epsC,D,C);

  Mat tmpM1, tmpM2;
  MatDuplicate(M,MAT_DO_NOT_COPY_VALUES,&tmpM1);
  MatDuplicate(M,MAT_DO_NOT_COPY_VALUES,&tmpM2);
  MatTranspose(M,MAT_REUSE_MATRIX,&M);
  MatMatMult(C,M,MAT_INITIAL_MATRIX,PETSC_DEFAULT,&tmpM1);
  MatMatMult(tmpM1,C,MAT_INITIAL_MATRIX,PETSC_DEFAULT,&tmpM2);
  eigsolver(tmpM2,epsC,D);
  MatDestroy(&tmpM1);
  MatDestroy(&tmpM2);
/*-------------------------------------------------------------------------*/
