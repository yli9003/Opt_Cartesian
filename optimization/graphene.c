...
/**************************************************************************************************************************************************************/
  ...
  double eps1x, eps1y, eps1z, eps2x, eps2y, eps2z, eps3x, eps3y, eps3z;
  ...
  PetscOptionsGetReal(PETSC_NULL,"-eps3x",&eps3x,&flg); MyCheckAndOutputDouble(flg,eps3x,"eps3x","eps3x");
  PetscOptionsGetReal(PETSC_NULL,"-eps3y",&eps3y,&flg); MyCheckAndOutputDouble(flg,eps3y,"eps3y","eps3y");
  PetscOptionsGetReal(PETSC_NULL,"-eps3z",&eps3z,&flg); MyCheckAndOutputDouble(flg,eps3z,"eps3z","eps3z");

  double epsair, epssub1, epssub2, epssub3;
  getreal("-epsair",&epsair,1);
  getreal("-epssub1",&epssub1,1);
  getreal("-epssub2",&epssub2,1);
  getreal("-epssub3",&epssub3,1);
...
/**************************************************************************************************************************************************************/
  ...
  MuinvPMLFull(PETSC_COMM_SELF, &muinvpml3,Nx,Ny,Nz,Npmlx,Npmly,Npmlz,sigmax,sigmay,sigmaz,omega3,LowerPML); 
  ...
  VecSet(epsIII,0.0);
  VecAXPY(epsIII,eps3x,unitx);
  VecAXPY(epsIII,eps3y,unity);
  VecAXPY(epsIII,eps3z,unitz); 
  ...
  EpsPMLFull(PETSC_COMM_WORLD, epspml3,Nx,Ny,Nz,Npmlx,Npmly,Npmlz,sigmax,sigmay,sigmaz,omega3, LowerPML);
  ...
  GetMediumVecwithSub2(epsmedium1,Nz,Mz,epsair,epssub1);
  GetMediumVecwithSub2(epsmedium2,Nz,Mz,epsair,epssub2);
  GetMediumVecwithSub2(epsmedium3,Nz,Mz,epsair,epssub3);
  //GetMediumVec(epsmedium1,Nz,Mz,epsair,epssub1);
  //GetMediumVec(epsmedium2,Nz,Mz,epsair,epssub2);
  //GetMediumVec(epsmedium3,Nz,Mz,epsair,epssub3);
...
/**************************************************************************************************************************************************************/
  ...
  VecSet(vecNL,0);
  int j, ix, iy, iz, ic, iq, ns, ne;
  ierr = VecGetOwnershipRange(vecNL, &ns, &ne); CHKERRQ(ierr);
  ...
  for (i = ns; i < ne; ++i) {
    ...
    if(iz==(Nz+Mz)/2-1){
      VecSetValue(vecNL, i, 1, INSERT_VALUES);
    }
    if(iz==(Nz+1)/2){
      VecSetValue(vecNL, i, 1, INSERT_VALUES);
    }
  }
  ierr = VecAssemblyBegin(vecNL); CHKERRQ(ierr);
  ierr = VecAssemblyEnd(vecNL); CHKERRQ(ierr);
  //OutputVec(PETSC_COMM_WORLD, vecNL, "vecNL",".m");  
  /***make vecNL ***/

  SFGdataGroupGraphene sfgdata={Nx,Ny,Nz,hxyz,epsSReal,epsFReal,omega1,omega2,omega3,M1,M2,M3,A,b1,b2,x1,x2,weightedJ1,weightedJ2,epspmlQ1,epspmlQ2,epspmlQ3,epsmedium1,epsmedium2,epsmedium3,epsI,epsII,epsIII,epscoef1,epscoef2,epscoef3,B1,B2,ksp1,ksp2,ksp3,&its1,&its2,&its3,p1,p2,outputbase,vecNL};
...
if (Job==1){
  ...
  nlopt_set_max_objective(opt,sfg_graphene,&sfgdata);
...
if (Job==2){
  ...
  { 
      epsopt[posMj]=epscen; 
      beta = sfg_graphene(DegFree,epsopt,grad,&sfgdata);
