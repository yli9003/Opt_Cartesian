...
/**************************************************************************************************************************************************************/
  ...
  if (!flg) sprintf(filenameComm,"chi2shg_");
...
if (Job==1){
  ...
  */
  ...
  LDOSdataGroupEpsOmega ldos1data = {0,M1,A,x1,b1,weightedJ1,epspmlQ,epsmedium,NULL,&its1,NULL,vgrad,ksp1,1};
  LDOSdataGroupEpsOmega ldos2data = {0,M2,A,x2,b2,weightedJ2,epspmlQ,epsmedium,NULL,&its2,NULL,vgrad,ksp2,1};
  LDOSdataGroupEpsOmega ldos3data = {0,M3,A,x3,b3,weightedJ3,epspmlQ,epsmedium,NULL,&its3,NULL,vgrad,ksp3,1};
  LDOSdataGroupEpsOmega ldos4data = {0,M4,A,x4,b4,weightedJ4,epspmlQ,epsmedium,NULL,&its4,NULL,vgrad,ksp4,1};
  LDOSdataGroupEpsOmega ldos5data = {0,M5,A,x5,b5,weightedJ5,epspmlQ,epsmedium,NULL,&its5,NULL,vgrad,ksp5,2};
  LDOSdataGroupEpsOmega ldos6data = {0,M6,A,x6,b6,weightedJ6,epspmlQ,epsmedium,NULL,&its6,NULL,vgrad,ksp6,3};

 /*---------Optimization--------*/
  double epsmin, epsmax;
  double f1min, f1max, f2min, f2max, f3min, f3max;
  getreal("-epsmin",&epsmin,4.5);
  getreal("-epsmax",&epsmax,11.5);
  getreal("-f1min",&f1min,0.5);
  getreal("-f1max",&f1max,0.6);
  getreal("-f2min",&f2min,0.35);
  getreal("-f2max",&f2max,0.41);
  getreal("-f3min",&f3min,0.66);
  getreal("-f3max",&f3max,0.80);
  double tstart;
  ...
  int DegFreeAll=DegFree+5;
  double *epsoptAll;
  epsoptAll = (double *) malloc(DegFreeAll*sizeof(double));
  for (i=0;i<DegFree;i++){ epsoptAll[i]=epsopt[i]; }
  epsoptAll[DegFreeAll-5]=tstart;
  epsoptAll[DegFreeAll-4]=eps1x;
  epsoptAll[DegFreeAll-3]=omega1/(2*PI);
  epsoptAll[DegFreeAll-2]=omega5/(2*PI);
  epsoptAll[DegFreeAll-1]=omega6/(2*PI);
  ...
  lb[DegFreeAll-5]=0;
  ub[DegFreeAll-5]=1.0/0.0;
  lb[DegFreeAll-4]=epsmin;
  ub[DegFreeAll-4]=epsmax;
  lb[DegFreeAll-3]=f1min;
  ub[DegFreeAll-3]=f1max;
  lb[DegFreeAll-2]=f2min;
  ub[DegFreeAll-2]=f2max;
  lb[DegFreeAll-1]=f3min;
  ub[DegFreeAll-1]=f3max;
  ...
  if(nummodes==1){
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos1data,1e-8);
  }else if(nummodes==2){
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos1data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos2data,1e-8);
  }else if(nummodes==3){
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos1data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos2data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos3data,1e-8);
  }else if(nummodes==4){
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos1data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos2data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos3data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos4data,1e-8);
  }else if(nummodes==5){
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos1data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos2data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos3data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos4data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos5data,1e-8);
  }else if(nummodes==6){
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos1data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos2data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos3data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos4data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos5data,1e-8);
    nlopt_add_inequality_constraint(opt,ldoskconstraintepsomega,&ldos6data,1e-8);
  }
  ...
  int doopt;
  getint("-doopt",&doopt,1);
  if(doopt){
    result = nlopt_optimize(opt,epsoptAll,&maxf);
  }else{
    int ncheck;
    double check, checkstart, checkend, dcheck;
    getint("-ncheck",&ncheck,1);
    getreal("-checkstart",&checkstart,0);
    getreal("-checkend",&checkend,1);
    getreal("-dcheck",&dcheck,0.01);
    
    ncheck=ncheck-1;
    double checkoutput;
    double *gradAll;
    gradAll = (double *) malloc(DegFreeAll*sizeof(double));
    for(check=checkstart;check<checkend;check=check+dcheck){
      epsoptAll[ncheck]=check;
      checkoutput=ldoskconstraintepsomega(DegFreeAll,epsoptAll,gradAll,&ldos1data);
      PetscPrintf(PETSC_COMM_WORLD,"----checked output: %.16e, %.16e, %.16e \n", check, checkoutput, gradAll[ncheck]);
    }
    result = -100;
  }
