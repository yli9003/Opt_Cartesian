...
if (Job==1){
 ...
 if (Job==2){
   ...
   int chiralmaxmin;
   double chiralweightL, chiralweightR;
   getint("-chiralmaxmin",&chiralmaxmin,0);
   getreal("-chiralweightL",&chiralweightL,1.0);
   getreal("-chiralweightR",&chiralweightR,1.0);
   ChiraldataGroup chiraldataL = {omega, M, A, xL, NULL, bL, NULL, weightedJL, NULL, epspmlQ, epsmedium, epsDiff,  &its, epscoef, vgrad, ksp, chiralweightL, 0};
   ChiraldataGroup chiraldataR = {omega, M, A, xR, NULL, bR, NULL, weightedJR, NULL, epspmlQ, epsmedium, epsDiff,  &its, epscoef, vgrad, ksp, chiralweightR, 1};

   if(chiralmaxmin==0){
     nlopt_set_max_objective(opt,ldoskdiff,&chiraldata);
   }else{
     nlopt_add_inequality_constraint(opt,ldoskchiralconstraint,&chiraldataL,1e-1);
     nlopt_add_inequality_constraint(opt,ldoskchiralconstraint,&chiraldataR,1e-1);
