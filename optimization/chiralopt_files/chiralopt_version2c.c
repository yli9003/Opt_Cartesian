...
if (Job==1){
 ...
 if (Job==2){
   ...
   int its2=100;
   ChiraldataGroup chiraldata2 = {omega, NULL, A, xL, xR, bL, bR, weightedJL, weightedJR, epspmlQ, epsmedium, epsDiff,  &its2, epscoef, vgrad, NULL, chiralweight, fomopt};
   if(chiralweight){
     Mat M2;
     KSP ksp2;
     PC pc2;
     double beta2[3]={-beta[1],beta[0],beta[1]};
     MoperatorGeneralBloch(PETSC_COMM_WORLD, &M2, Nx,Ny,Nz, hx,hy,hz, bx,by,bz, muinv, BCPeriod, beta2);
     setupKSP(PETSC_COMM_WORLD,&ksp2,&pc2,solver,iteronly);
     chiraldata2.M=M2;
     chiraldata2.its=&its2;
     chiraldata2.ksp=ksp2;

     nlopt_add_inequality_constraint(opt,ldoskdiff,&chiraldata ,1e-8);
     nlopt_add_inequality_constraint(opt,ldoskdiff,&chiraldata2,1e-8);
