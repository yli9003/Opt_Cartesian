...
/**************************************************************************************************************************************************************/
  ...
  int devicedir[nfreq];
  double refphi[nfreq*nangle], slope[nfreq];
  ...
  for(i=0;i<nfreq;i++){
    ...
    sprintf(tmpflg,"-slope[%d]",i+1);
    getreal(tmpflg,slope+i,0);
...
/**************************************************************************************************************************************************************/
  ...
  for(i=0;i<nfreq;i++){
    ...
    for(j=0;j<nangle;j++){
      ...
      makepq_linear(PETSC_COMM_WORLD,pvec+jpt,qvec+jpt, Nx,Ny,Nz, rlx,rux,rly,ruy,rlz,ruz, devicedir[i], 1/(freq[i]*hz),refphi[jpt], rx0,ry0,rz0, slope[jpt]);
    }
